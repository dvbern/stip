...
export class BerechnungsExpansionPanelComponent {
  ...
  viewSig = computed(() => {
    ...
    return {
      titleKey: translatableShared(
        `shared.verfuegung.berechnung.${berechnung.typ}.${variant}.title`,
      ),
      infoKey: translatableShared(
        `shared.verfuegung.berechnung.${berechnung.typ}.${variant}.total`,
      ),
