...
ALTER TABLE benutzer
  ADD COLUMN benutzereinstellungen_id UUID;
ALTER TABLE benutzer_aud
  ADD COLUMN benutzereinstellungen_id UUID;

WITH benutzers AS (SELECT *
                   FROM benutzer)
INSERT
INTO benutzereinstellungen(id, timestamp_erstellt, timestamp_mutiert, user_erstellt, user_mutiert, version,
...
UPDATE benutzer
SET benutzereinstellungen_id = bs.id
FROM benutzereinstellungen AS bs;

ALTER TABLE benutzer
  ALTER COLUMN benutzereinstellungen_id SET NOT NULL;

ALTER TABLE person_in_ausbildung
  DROP COLUMN digitale_kommunikation;
